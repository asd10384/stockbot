...
export default async function searchstock(name: string): Promise<[ market, stocknametypes ] | undefined> {
  ...
  if (getlist && getlist[0]?.description) getlist[0].description = getlist[0].description.trim().endsWith("보통주") ? getlist[0].description.replace("보통주", "") : getlist[0].description;
